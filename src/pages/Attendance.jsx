      ...
      <div className="px-4 sm:px-6 md:px-10 py-6 md:py-8">
        <div className="mb-6 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3">
          <div className="flex items-center gap-3">
            <label className="text-xs font-medium text-ink-700 dark:text-chalk-bg/90 flex-shrink-0">📅 {t('att_date')}</label>
            <input
              type="date"
              value={date}
              onChange={(e) => setDate(e.target.value)}
              className="bg-chalk-card dark:bg-board-800 text-ink-900 dark:text-white flex-1 sm:flex-none border border-chalk-line dark:border-board-700 rounded-card px-3 py-2 sm:py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-board-600 dark:focus:ring-gold-500"
            />
          </div>
          <div className="flex items-center gap-3">
            <label className="text-xs font-medium text-ink-700 dark:text-chalk-bg/90 flex-shrink-0">{t('att_selectYear')}</label>
            <select
              value={markYear}
          ...
          {markGrouped.map(([yearKey, list]) => (
            <div key={yearKey}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-ink-700/50 dark:text-chalk-bg/50">{list.length} {t('att_students')}</span>
              </div>
              <Card>
                  ...
                  {list.map((s) => {
                    ...
                    return (
                      <div key={s.id} className="p-4">
                        ...
                        <div className="flex rounded-card border border-chalk-line dark:border-board-700 overflow-hidden">
                          <button
                            onClick={() => toggle(s.id, true)}
                            className={`flex-1 py-2 text-xs font-medium transition ${
                              present === true
                                ? 'bg-pass-bar text-white'
                                : 'bg-chalk-card dark:bg-board-800 text-ink-700/60 dark:text-chalk-bg/60 active:bg-pass-bg'
                            }`}
                          >
                            {t('att_present')}
                          </button>
                          <button
                            ...
                            className={`flex-1 py-2 text-xs font-medium transition border-l border-chalk-line dark:border-board-700 ${
                              present === false
                                ? 'bg-fail-bar text-white'
                                : 'bg-chalk-card dark:bg-board-800 text-ink-700/60 dark:text-chalk-bg/60 active:bg-fail-bg'
                            }`}
                          >
                            {t('att_absent')}
                          </button>
                        ...
                        return (
                          <tr key={s.id} className="border-b border-chalk-line dark:border-board-700 last:border-0">
                            ...
                            <td className="px-4 py-2.5">
                              <div className="inline-flex rounded-card border border-chalk-line dark:border-board-700 overflow-hidden">
                                <button
                                  onClick={() => toggle(s.id, true)}
                                  className={`px-3 py-1 text-xs font-medium transition ${
                                    present === true
                                      ? 'bg-pass-bar text-white'
                                      : 'bg-chalk-card dark:bg-board-800 text-ink-700/60 dark:text-chalk-bg/60 hover:bg-pass-bg'
                                  }`}
                                >
                                  {t('att_present')}
                                </button>
                                <button
                                  onClick={() => toggle(s.id, false)}
                                  className={`px-3 py-1 text-xs font-medium transition border-l border-chalk-line dark:border-board-700 ${
                                    present === false
                                      ? 'bg-fail-bar text-white'
                                      : 'bg-chalk-card dark:bg-board-800 text-ink-700/60 dark:text-chalk-bg/60 hover:bg-fail-bg'
                                  }`}
                                >
                                  {t('att_absent')}
                                </button>
                            ...
                            <td className="px-4 py-2.5">
                              <AttendanceStamp rate={rate} />
                              <span className="text-[11px] text-ink-700/40 ml-2">
                                {records.length} {t('att_daysRecorded')}
                              </span>
                            </td>
                          </tr>
        ...
        <div className="mt-10 pt-8 border-t border-chalk-line dark:border-board-700">
          <h2 className="font-display text-2xl md:text-3xl text-ink-900 dark:text-white">{t('att_historyTitle')}</h2>
          <p className="text-sm text-ink-700/70 dark:text-chalk-bg/70 mt-1.5 mb-6">{t('att_historySubtitle')}</p>

          <Card className="p-4 mb-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="col-span-2 sm:col-span-1">
                <label className="block text-[11px] font-medium text-ink-700 dark:text-chalk-bg/90 mb-1">
                  {t('att_selectYear')}
                </label>
                <select
                  value={historyYear}
              ...
              <div>
                ...
                <input
                  ...
                  className="bg-chalk-card dark:bg-board-800 text-ink-900 dark:text-white w-full border border-chalk-line dark:border-board-700 rounded-card px-2 sm:px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-board-600 dark:focus:ring-gold-500"
                />
              </div>
              <div>
                <label className="block text-[11px] font-medium text-ink-700 dark:text-chalk-bg/90 mb-1">
            ...
            {(fromDate || toDate) && (
              <button
                ...
                className="font-mono-tag text-xs text-ink-600 dark:text-gold-500 border border-chalk-line dark:border-board-700 rounded-card px-3 py-1.5 mt-3 hover:bg-chalk-bg dark:bg-board-900 transition"
              >
                ✕ {t('dash_clearFilters')}
              </button>
            )}
          ...
          {historyYear && historyDates.length > 0 && (
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              <button
                onClick={exportCSV}
                className="font-mono-tag text-xs border border-board-600 text-ink-700 dark:text-chalk-bg/90 rounded-card px-3 py-2 sm:py-1.5 hover:bg-board-800 hover:text-white hover:border-board-800 transition"
              >
                ⬇ {t('att_exportSheet')}
              </button>
            </div>
            ...
            <Card>
              <div className="overflow-x-auto">
                <table className="text-sm">
                  <thead>
                    <tr className="text-left text-xs text-ink-700/60 dark:text-chalk-bg/60 border-b border-chalk-line dark:border-board-700">
                      <th className="px-4 py-2 font-medium sticky left-0 bg-chalk-card dark:bg-board-800">{t('att_name')}</th>
                  ...
                  <tbody>
                    {historyStudents.map((s) => {
                      ...
                      return (
                        <tr key={s.id} className="border-b border-chalk-line dark:border-board-700 last:border-0">
                          <td className="px-4 py-2.5 whitespace-nowrap sticky left-0 bg-chalk-card dark:bg-board-800">
                            {s.name}
                          </td>
                          <td className="px-4 py-2.5 text-ink-700/70 dark:text-chalk-bg/70 whitespace-nowrap">{s.phone || '—'}</td>
                          <td className="px-4 py-2.5 text-ink-700/70 dark:text-chalk-bg/70 whitespace-nowrap">{s.school || '—'}</td>
                          {historyDates.map((d) => {
                            const val = attendanceMatrix[s.id]?.[d]
                            return (
